...
MEMORY_ACCOUNT m_hist;

void histogram_init()
{
  init_memory_account(&m_hist,0);
}

void delete_histograms()
{
  clear_memory_account(&m_hist);
}
...
{
  int i;
  hist->v=get_memory(&m_hist,sizeof(double)*ncx*ncy);
  hist->v2=get_memory(&m_hist,sizeof(double)*ncx*ncy);
  hist->c=get_memory(&m_hist,sizeof(long)*ncx*ncy);
  for (i=0;i<ncx*ncy;i++)
    {
      hist->v[i]=0.0;
      hist->v2[i]=0.0;
...
{
  int ix,iy;
  ix=(x-hist->xmin)*hist->dx_i;
  if ((ix<0)||(ix>=hist->ncx)) return;
  iy=(y-hist->ymin)*hist->dy_i;
		    ...
		    char *name)
{
  ...
  switch (type)
    ...
    case 1:
      ...
      histogram->xmax=xmax;
      histogram->type=type;
      histogram->y=get_memory(&m_hist,sizeof(double)*n);
      for (i=0;i<n;i++)
	{
	  histogram->y[i]=0.0;
	}
    ...
    case 3:
      ...
      histogram->type=type;
      histogram->y=get_memory(&m_hist,sizeof(double)*n);
      histogram->count=get_memory(&m_hist,sizeof(long)*n);
      for (i=0;i<n;i++)
	{
	  histogram->y[i]=0.0;
	  histogram->count[i]=0;
    ...
    case 4:
      ...
      histogram->type=type;
      histogram->y=get_memory(&m_hist,sizeof(double)*n);
      histogram->y2=get_memory(&m_hist,sizeof(double)*n);
      histogram->count=get_memory(&m_hist,sizeof(long)*n);
      for (i=0;i<n;i++)
	{
	  histogram->y[i]=0.0;
...
void inquire_histogram(HISTOGRAM *histogram,int *type,float *xmin,float *xmax,
		       int *n)
{
  *type=histogram->type;
...
set_histogram_channel_y(HISTOGRAM *histogram,int channel,double value)
{
  histogram->y[channel]=value;
}

set_histogram_channel_y2(HISTOGRAM *histogram,int channel,double value)
{
  histogram->y2[channel]=value;
}

set_histogram_channel_count(HISTOGRAM *histogram,int channel,int value)
{
...
void compare_histogram(HISTOGRAM *hist1,HISTOGRAM *hist2,HISTOGRAM *hist)
{
  float eps=1e-10;
  int n1,n2,type1,type2,i,c1,c2;
  float xmin1,xmin2,xmax1,xmax2,h1,h2;
  inquire_histogram(hist1,&type1,&xmin1,&xmax1,&n1);
  inquire_histogram(hist2,&type2,&xmin2,&xmax2,&n2);
  if ((type1!=type2)||(xmin1!=xmin2)||(xmax1!=xmax2)||(n1!=n2))
    {
      return;
    }
  inquire_histogram(hist,&type2,&xmin2,&xmax2,&n2);
  if ((type1!=type2)||(xmin1!=xmin2)||(xmax1!=xmax2)||(n1!=n2))
    {
      return;
    }
  for (i=0;i<n1;i++)
    {
      ...
      set_histogram_channel_y(hist,i,(h1-h2)/h2);
      c2=histogram_count(hist2,i);
      set_histogram_channel_y(hist,i,c2);
      h2=histogram_y2(hist2,i);
      set_histogram_channel_y2(hist,i,h2/h1);
    }
}

void histogram_type_copy(HISTOGRAM *hist0,HISTOGRAM *hist)
{
  inquire_histogram(hist0,&hist->type,&hist->xmin,&hist->xmax,&(hist->nc));
      ...
      if (((i+1) % n)==0)
        {
          switch (hist->type)
            {
	    case 4:
              set_histogram_channel_y2(hist,j,x2);
	      x2=0.0;
	    case 3:
              set_histogram_channel_count(hist,j,c);
	      c=0;
	    case 2:
	    case 1:
              set_histogram_channel_y(hist,j,x);
              x=0.0;
	      j++;
            }
        }
  ...
  for (i=0;i<n1;i++)
	...
	case 3:
     	  c1=histogram_count(hist1,i);
      	  c2=histogram_count(hist2,i);
      	  set_histogram_channel_count(hist,i,c1+c2);
	case 1:
	case 2:
	  ...
	  h=max(eps,h1+h2);
h=max(h2,eps);
      	  set_histogram_channel_y(hist,i,(h1-h2)/h);
          if(hist->type==4)
            {
              h1_2=histogram_y2(hist1,i);
      	      h2_2=histogram_y2(hist2,i);
/*      	      set_histogram_channel_y2(hist,i,
	2.0*(h2*h2+h1*h1)*(h1_2+h2_2)/(h*h*h*h));*/
      	      set_histogram_channel_y2(hist,i,(h1_2+h2_2)/(h*h));
/*      	      set_histogram_channel_y2(hist,i,h1_2+h2_2);*/
            }
        }
    }
}

void store_histogram(FILE *datei,HISTOGRAM *histogram)
{
  int i;
  ...
  switch (histogram->type)
      ...
      for (i=0;i<histogram->nc;i++)
	{
	  ...
	  fprintf(datei,"%15d %15e",histogram->count[i],histogram->y[i]);
	}
      fprintf(datei,"\n");
      break;
    case 4:
      for (i=0;i<histogram->nc;i++)
	{
	  ...
	  fprintf(datei,"%15d %15e %15e",histogram->count[i],histogram->y[i],
		  histogram->y2[i]);
	}
      fprintf(datei,"\n");
      break;
...
double sum_histogram(HISTOGRAM *histogram)
{
  int i;
  double sum=0.0;
...
void density_histogram(HISTOGRAM *histogram)
{
  int i;
...
void fprint_histogram(FILE *datei,HISTOGRAM *histogram)
{
  int i;
  float xstep;
...
{
  fprint_histogram(stdout,histogram);
}

void fprint_histogram_2(FILE *datei,HISTOGRAM *hist1,HISTOGRAM *hist2)
{
  int i;
  float xstep;
  switch (hist1->type)
...
void make_error_histogram(HISTOGRAM *hist,HISTOGRAM *hist_err)
{
  int i;
  for (i=0;i<hist->nc;i++)
    {
...
void fitprint_histogram(FILE *datei,HISTOGRAM *histogram)
{
  int i;
  float xstep;
  double sigma,eps=1e-10;
